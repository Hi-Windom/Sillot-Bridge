...
import { hasClosestBlock, hasClosestByAttribute, hasClosestByClassName, hasClosestByMatchTag, hasTopClosestByClassName, hasTopClosestByTag, } from "../protyle/util/hasClosest";
...
const getRightBlock = (element, x, y) => {
    let index = 1;
    let nodeElement = element;
    while (nodeElement && (nodeElement.classList.contains("list") || nodeElement.classList.contains("li"))) {
        nodeElement = document.elementFromPoint(x + 73 * index, y);
        nodeElement = hasClosestBlock(nodeElement);
        index++;
    }
    return nodeElement;
};
const switchDialogEvent = (event, switchDialog) => {
...
export const globalShortcut = () => {
    ...
    window.addEventListener("mousemove", (event) => {
        ...
        const eventPath0 = event.composedPath()[0];
        ...
        if (blockElement && blockElement.style.cursor !== "col-resize" && !hasClosestByClassName(blockElement, "protyle-wysiwyg__embed")) {
            const cellElement = (hasClosestByMatchTag(target, "TH") || hasClosestByMatchTag(target, "TD"));
            if (cellElement) {
    ...
    <div class="switch-doc__path"></div>
        ...
        if (matchHotKey(window.siyuan.config.keymap.general.closeTab.custom, event) && !event.repeat) {
            ...
            if (activeTabElement) {
                const tab = getInstanceById(activeTabElement.getAttribute("data-id"));
    ...
    window.addEventListener("click", (event) => {
        ...
        if (!currentPDFViewerObject.supportsIntegratedFind &&
            currentPDFViewerObject.findBar.opened) {
            currentPDFViewerObject.findBar.close();
        }
    });
};
const dialogArrow = (element, event) => {
...
const fileTreeKeydown = (event) => {
    ...
    if (matchHotKey("⌘/", event)) {
        ...
        if (isFile) {
            ...
            initNavigationMenu(liElements[0]).popup({ x: liRect.right - 15, y: liRect.top + 15 });
        }
        return true;
    }
    ...
    if (event.shiftKey) {
        if (event.key === "ArrowUp") {
            ...
            if (startEndElement.startElement.getBoundingClientRect().top >= startEndElement.endElement.getBoundingClientRect().top) {
                previousElement = getPreviousFileLi(startEndElement.endElement);
                if (previousElement) {
            ...
            let nextElement;
            if (startEndElement.startElement.getBoundingClientRect().top <= startEndElement.endElement.getBoundingClientRect().top) {
                nextElement = getNextFileLi(startEndElement.endElement);
                if (nextElement) {
...
const panelTreeKeydown = (event) => {
    ...
    const model = getInstanceById(activePanelElement.getAttribute("data-id"), window.siyuan.layout.layout)?.model;
    if (!model) {
        return false;
    }
