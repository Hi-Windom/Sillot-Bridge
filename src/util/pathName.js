...
export const getAssetName = (assetPath) => {
...
export const getTopPaths = (liElements) => {
    const fromPaths = [];
...
export const moveToPath = (fromPaths, toNotebook, toPath) => {
...
export const movePathTo = (cb, paths, range, title) => {
...
<li class="b3-list-item${html === "" ? " b3-list-item--focus" : ""}" data-path="/" data-box="${item.id}">
    ...
    inputElement.addEventListener("keydown", (event) => {
        ...
        let currentItemElement = currentItemElements[0];
...
export const getNotebookIcon = (id) => {
    let rootPath = "";
    window.siyuan.notebooks.find((item) => {
        if (item.id === id) {
            rootPath = item.icon;
