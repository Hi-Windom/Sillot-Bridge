...
const renderCompare = (element) => {
    ...
    if (!leftEditor) {
        leftEditor = new Protyle(leftElement.lastElementChild, {
            blockId: "",
        ...
        rightEditor = new Protyle(rightElement.lastElementChild, {
            blockId: "",
            action: [Constants.CB_GET_HISTORY],
            render: {
                background: false,
    ...
    fetchPost("/api/repo/openRepoSnapshotDoc", { id: element.getAttribute("data-id") }, (response) => {
        ...
        const textElement = leftElement.firstElementChild.nextElementSibling;
    ...
    if (id2) {
        ...
        fetchPost("/api/repo/openRepoSnapshotDoc", { id: id2 }, (response) => {
            const textElement = rightElement.firstElementChild.nextElementSibling;
            if (response.data.isLargeDoc) {
                textElement.value = response.data.content;
                textElement.classList.remove("fn__none");
