...
let historyEditor;
const renderDoc = (element, currentPage) => {
    ...
    const opElement = element.querySelector('.b3-select[data-type="opselect"]');
    const typeElement = element.querySelector('.b3-select[data-type="typeselect"]');
    const notebookElement = element.querySelector('.b3-select[data-type="notebookselect"]');
    ...
    const mdElement = element.querySelector('.history__text[data-type="mdPanel"]');
...
const renderRepoItem = (response, element, type) => {
...
const renderRepo = (element, currentPage) => {
...
const renderRmNotebook = (element) => {
    element.setAttribute("data-init", "true");
    fetchPost("/api/history/getNotebookHistory", {}, (response) => {
        ...
        let logsHTML = "";
        response.data.histories.forEach((item, index) => {
...
export const openHistory = () => {
    ...
    const firstPanelElement = dialog.element.querySelector("#historyContainer [data-type=doc]");
    ...
    firstPanelElement.querySelector(".b3-text-field").addEventListener("input", (event) => {
    ...
    const docElement = firstPanelElement.querySelector('.history__text[data-type="docPanel"]');
    const assetElement = firstPanelElement.querySelector('.history__text[data-type="assetPanel"]');
    const mdElement = firstPanelElement.querySelector('.history__text[data-type="mdPanel"]');
    renderDoc(firstPanelElement, 1);
    ...
    const selectElement = repoElement.querySelector(".b3-select");
    ...
    dialog.element.addEventListener("click", (event) => {
        ...
        while (target && !target.isEqualNode(dialog.element)) {
            ...
            if (target.classList.contains("item")) {
                ...
                Array.from(dialog.element.querySelector("#historyContainer").children).forEach((item) => {
                ...
                confirmDialog("⚠️ " + window.siyuan.languages.rollback, `${window.siyuan.languages.rollbackConfirm.replace("${date}", target.parentElement.textContent.trim())}`, () => {
                    ...
                    if (dataType === "assets") {
                        ...
                        fetchPost("/api/history/rollbackDocHistory", {
                            notebook: firstPanelElement.querySelector('.b3-select[data-type="notebookselect"]').value,
                            historyPath: target.parentElement.getAttribute("data-path")
                        });
                    }
                ...
                if (iconElement.classList.contains("b3-list-item__arrow--open")) {
                    ...
                    if (target.nextElementSibling && target.nextElementSibling.tagName === "UL") {
                        ...
                        const inputElement = firstPanelElement.querySelector(".b3-text-field");
                        const opElement = firstPanelElement.querySelector('.b3-select[data-type="opselect"]');
                        const typeElement = firstPanelElement.querySelector('.b3-select[data-type="typeselect"]');
                        const notebookElement = firstPanelElement.querySelector('.b3-select[data-type="notebookselect"]');
        ...
        <svg><use xlink:href="#iconUndo"></use></svg>
                ...
                if (type === "assets") {
                    ...
                    fetchPost("/api/history/getDocHistoryContent", {
                        ...
                        k: firstPanelElement.querySelector(".b3-text-field").value
                    }, (response) => {
                ...
                let currentItem = hasClosestByClassName(target, "b3-list");
                if (currentItem) {
...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.tagSnapshotUpload}</button>
                ...
                const inputElement = genTagDialog.element.querySelector(".b3-text-field");
                inputElement.select();
