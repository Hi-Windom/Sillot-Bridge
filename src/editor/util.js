...
export const openAsset = (assetPath, page, position) => {
    openFile({
        assetPath,
        page,
        position,
...
const switchEditor = (editor, options, allModels) => {
    ...
    if ((!nodeElement || nodeElement?.clientHeight === 0) && options.id !== options.rootID) {
        ...
        if (options.action.includes(Constants.CB_GET_HL)) {
            ...
            if (nodeElement) {
                ...
                nodeElement = hasClosestBlock(editor.editor.protyle.toolbar.range.startContainer);
...
export const updatePanelByEditor = (protyle, focus = true, pushBackStack = false, reload = false) => {
...
export const isCurrentEditor = (blockId) => {
...
const updateOutline = (models, protyle, reload = false) => {
...
export const updateBacklinkGraph = (models, protyle) => {
    // https://ld246.com/article/1637636106054/comment/1641485541929#comments
    if (protyle?.element.classList.contains("fn__none") ||
