...
export const deleteFile = (notebookId, pathString, name) => {
    ...
    if (liElements.length === 1) {
        ...
        if (itemTopULElement) {
            ...
            if (liElements[0].getAttribute("data-type") === "navigation-file") {
                ...
                confirmDialog(window.siyuan.languages.deleteOpConfirm, `${window.siyuan.languages.confirmDelete} <b>${Lute.EscapeHTMLStr(getNotebookName(itemNotebookId))}</b>?`, () => {
        ...
        confirmDialog(window.siyuan.languages.deleteOpConfirm, window.siyuan.languages.confirmRemoveAll, () => {
