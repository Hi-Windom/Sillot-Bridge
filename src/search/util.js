...
const saveKeyList = (type, value) => {
    let list = window.siyuan.storage[Constants.LOCAL_SEARCHEKEYS][type];
...
// closeCB 不存在为页签搜索
export const genSearch = (config, element, closeCB) => {
...
<div class="fn__loading fn__loading--top"><img width="120px" src="/stage/loading-pure.svg"></div>`;
    ...
    const replaceInputElement = element.querySelector("#replaceInput");
    ...
    const lineHeight = 30;
    const edit = new Protyle(element.querySelector("#searchPreview"), {
        blockId: "",
        render: {
            gutter: true,
    ...
    element.addEventListener("click", (event) => {
        ...
        while (target && !target.isSameNode(element)) {
            if (target.classList.contains("search__rmpath")) {
                ...
                if (element) {
                    wnd = getInstanceById(element.getAttribute("data-id"));
                }
                if (!wnd) {
                    wnd = getWndByLayout(window.siyuan.layout.centerLayout);
                }
    ...
    searchInputElement.addEventListener("compositionend", (event) => {
    ...
    searchInputElement.addEventListener("keydown", (event) => {
        let currentList = searchPanelElement.querySelector(".b3-list-item--focus");
...
const addConfigMoreMenu = async (config, edit, element, event) => {
...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
            ...
            btnsElement[1].addEventListener("click", () => {
                ...
                config.k = element.querySelector("#searchInput").value;
                config.r = element.querySelector("#replaceInput").value;
    ...
    <svg class="b3-menu__icon fn__a" style="width: 8px"><use xlink:href="#iconClose"></use></svg>
            ...
            bind(menuElement) {
                menuElement.addEventListener("click", (event) => {
                    if (hasClosestByClassName(event.target, "fn__a")) {
...
const updateConfig = (element, item, config, edit) => {
    ...
    element.querySelector("#searchInput").value = item.k;
    element.querySelector("#replaceInput").value = item.r;
...
const addConfigFilterMenu = (config, edit, element) => {
...
<div class="b3-dialog__action">
    ...
    btnsElement[1].addEventListener("click", () => {
        filterDialog.element.querySelectorAll(".b3-switch").forEach((item) => {
            config.types[item.getAttribute("data-type")] = item.checked;
        });
        inputEvent(element, config, undefined, edit);
        filterDialog.destroy();
    });
};
const addQueryMenu = (config, edit, element) => {
...
const replace = (element, config, edit, isAll) => {
    ...
    fetchPost("/api/search/findReplace", {
        k: config.method === 0 ? getKey(currentList) : element.querySelector("#searchInput").value,
...
const inputEvent = (element, config, inputTimeout, edit, event) => {
...
const onSearch = (data, edit, element) => {
    let resultHTML = "";
