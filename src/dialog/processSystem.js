...
export const progressLoading = (data) => {
    ...
    if (data.code === 0) {
        progressElement.innerHTML = /*html*/ `<div class="b3-dialog__scrim" style="z-index:400;opacity: 1"></div>
...
export const progressBackgroundTask = (tasks) => {
...
export const downloadProgress = (data) => {
    const bazzarElement = document.getElementById("configBazaarReadme");
    if (!bazzarElement) {
        return;
    }
    const btnElement = bazzarElement.querySelector('[data-type="install"]');
    if (btnElement) {
        if (data.percent >= 1) {
