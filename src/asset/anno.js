...
export const initAnno = (file, element, annoId, pdf, pdfConfig) => {
    getConfig(pdf);
    ...
    pdfConfig.mainContainer.addEventListener("mousedown", (event) => {
        ...
        documentSelf.onmousemove = (moveEvent) => {
            ...
            rectResizeElement.setAttribute("style", `top:${newTop}px;height:${newHeight}px;left:${newLeft}px;width:${newWidth}px;background-color:${moveEvent.altKey ? "var(--b3-pdf-background1)" : ""}`);
        };
    ...
    element.addEventListener("click", (event) => {
        ...
        while (target && !target.classList.contains("pdf__outer")) {
            ...
            if (target.classList.contains("color__square")) {
                ...
                if (rectElement) {
                    ...
                    Array.from(rectElement.children).forEach((item) => {
                    ...
                    if (pdfConfig.mainContainer.lastElementChild.classList.contains("fn__none")) {
                        ...
                        coords = getHightlightCoordsByRect(pdf, color, pdfConfig.mainContainer.lastElementChild, pdfConfig.mainContainer.lastElementChild.style.backgroundColor ? "text" : "border");
                    ...
                    if (coords) {
                        coords.forEach((item, index) => {
                            ...
                            if (index === 0) {
                                ...
                                copyAnno(`${pdf.appConfig.file.replace(location.origin, "").substr(1)}/${rectElement.getAttribute("data-node-id")}`, pdf.appConfig.file.replace(location.origin, "").substr(8).replace(/-\d{14}-\w{7}.pdf$/, ""));
                            }
                        });
                    }
                }
        ...
        setTimeout(() => {
            const rectIsResize = !pdfConfig.mainContainer.lastElementChild.classList.contains("fn__none");
            let isShow = false;
            ...
            if (!isShow) {
                if (rectIsResize) {
                    if (!hasClosestByClassName(event.target, "pdf__util") &&
                        !hasClosestByClassName(event.target, "pdf__resize") &&
                        pdfConfig.mainContainer.lastElementChild.getAttribute("data-render") !== "true") {
                        hideToolbar(element);
                        pdfConfig.mainContainer.lastElementChild.classList.add("fn__none");
                    }
...
let rectElement;
const showToolbar = (element, range, target) => {
    if (target) {
        // 阻止 popover
...
const getHightlightCoordsByRange = (pdf, color) => {
    const range = window.getSelection().getRangeAt(0);
    const startPageElement = hasClosestByClassName(range.startContainer, "page");
    if (!startPageElement) {
        return;
    }
    const startIndex = parseInt(startPageElement.getAttribute("data-page-number")) - 1;
    ...
    mergeRects(range).forEach(function (r) {
        startSelected.push(startViewport.convertToPdfPoint(r.left - startPageRect.x, r.top - startPageRect.y).concat(startViewport.convertToPdfPoint(r.right - startPageRect.x, r.bottom - startPageRect.y)));
    });
    const endSelected = [];
    if (startIndex !== endIndex) {
        ...
        mergeRects(cloneRange).forEach(function (r) {
            endSelected.push(endViewport.convertToPdfPoint(r.left - endPageRect.x, r.top - endPageRect.y).concat(endViewport.convertToPdfPoint(r.right - endPageRect.x, r.bottom - endPageRect.y)));
        });
    }
...
const getHightlightCoordsByRect = (pdf, color, rectResizeElement, type) => {
    ...
    const startPageRect = startPage.canvas.getClientRects()[0];
    const startViewport = startPage.viewport;
    const startSelected = startViewport.convertToPdfPoint(rect.left - startPageRect.x, rect.top - startPageRect.y).concat(startViewport.convertToPdfPoint(rect.right - startPageRect.x, rect.bottom - startPageRect.y));
    const pages = [
        {
    ...
    endPageElement = hasClosestByClassName(endPageElement, "page");
    if (endPageElement) {
        const endIndex = parseInt(endPageElement.getAttribute("data-page-number")) - 1;
        if (endIndex !== startIndex) {
            ...
            const endPageRect = endPage.canvas.getClientRects()[0];
            const endViewport = endPage.viewport;
            const endSelected = endViewport.convertToPdfPoint(rect.left - endPageRect.x, rect.top - endPageRect.y).concat(endViewport.convertToPdfPoint(rect.right - endPageRect.x, rect.bottom - endPageRect.y));
            pages.push({
                index: endPage.id - 1,
                positions: [endSelected],
            });
...
export const getPdfInstance = (element) => {
    let pdfInstance;
...
export const getHighlight = (element) => {
    const pdfInstance = getPdfInstance(element);
    if (!pdfInstance) {
        return;
    }
    ...
    Object.keys(config).find(key => {
        ...
        const page = item.pages.find((page) => {
...
const showHighlight = (selected, pdf, hl) => {
...
export const hlPDFRect = (element, id) => {
    const currentElement = element.querySelector(`.pdf__rect[data-node-id="${id}"]`);
    if (currentElement?.firstElementChild) {
        const scrollElement = hasClosestByAttribute(currentElement, "id", "viewerContainer");
        if (scrollElement) {
...
const copyAnno = (idPath, fileName) => {
