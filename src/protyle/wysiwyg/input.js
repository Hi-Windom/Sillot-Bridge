...
export const input = async (protyle, blockElement, range, needRender = true) => {
    ...
    if (brElement && brElement.parentElement.tagName !== "TD" && brElement.parentElement.tagName !== "TH" && (brElement.parentElement.textContent.trim() === "" ||
    ...
    if (needRender && (getContenteditableElement(tempElement.content.firstElementChild)?.innerHTML !== getContenteditableElement(blockElement).innerHTML ||
        ...
        protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${id}"]`).forEach((item) => {
...
const updateInput = (html, protyle, id, type, oldHTML) => {
    const tempElement = document.createElement("template");
    tempElement.innerHTML = html;
    const doOperations = [];
