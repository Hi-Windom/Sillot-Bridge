...
import { focusBlock, focusByOffset, getSelectionOffset, setFirstNodeRange, } from "../util/selection";
...
export const commonHotkey = (protyle, event) => {
...
export const getStartEndElement = (selectElements) => {
    let startElement;
    let endElement;
...
export const duplicateBlock = (nodeElements, protyle) => {
    let focusElement;
    const doOperations = [];
...
export const alignImgCenter = (protyle, nodeElement, assetElements, id, html) => {
...
export const alignImgLeft = (protyle, nodeElement, assetElements, id, html) => {
    nodeElement.setAttribute("updated", format(new Date(), 'yyyyMMddHHmmss'));
    assetElements.forEach((item) => {
        item.style.display = "";
