...
const removeTopElement = (updateElement, protyle) => {
    // 移动到其他文档中，该块需移除
...
const promiseTransaction = () => {
    ...
    fetchPost("/api/transactions", {
        ...
        response.data[0].doOperations.forEach((operation) => {
...
const updateEmbed = (protyle, operation) => {
...
// 用于推送和撤销
export const onTransaction = (protyle, operation, focus) => {
    const updateElements = [];
    Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${operation.id}"]`)).forEach(item => {
    ...
    if (operation.action === "foldHeading") {
        ...
        if (operation.retData) {
            operation.retData.forEach((item) => {
        ...
        if (updateElements.length === 0) {
            ...
            getAllModels().editor.forEach(editor => {
                ...
                if (updateCloneElement) {
                    updateElements.push(updateCloneElement.cloneNode(true));
                }
            });
        }
        if (updateElements.length === 0) {
...
const removeUnfoldRepeatBlock = (html, protyle) => {
...
export const turnsIntoTransaction = (options) => {
    let selectsElement = options.selectsElement;
    let range;
    // 通过快捷键触发
...
const updateRef = (protyle, id, index = 0) => {
    if (index > 6) {
        return;
    }
...
export const transaction = (protyle, doOperations, undoOperations) => {
...
export const updateTransaction = (protyle, id, newHTML, html) => {
    if (newHTML === html) {
        return;
    }
    transaction(protyle, [{
...
export const updateBatchTransaction = (nodeElements, protyle, cb) => {
    const operations = [];
    const undoOperations = [];
