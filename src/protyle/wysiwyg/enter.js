...
import { getContenteditableElement, getTopEmptyElement, hasNextSibling, hasPreviousSibling, isNotEditBlock } from "./getBlock";
...
const listEnter = (protyle, blockElement, range) => {
...
export const enter = (blockElement, range, protyle) => {
    ...
    if (blockElement.parentElement.getAttribute("data-type") === "NodeListItem" &&
        (blockElement.nextElementSibling.classList.contains("protyle-attr") ||
            ...
            blockElement.parentElement.getAttribute("fold") === "1") && listEnter(protyle, blockElement, range)) {
        return true;
    }
    // 段首换行
...
const removeEmptyNode = (newElement) => {
