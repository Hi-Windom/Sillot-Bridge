...
import { hasClosestBlock, hasClosestByAttribute, hasClosestByClassName, hasClosestByMatchTag, hasTopClosestByClassName, } from "../util/hasClosest";
import { focusBlock, focusByRange, focusByWbr, focusSideBlock, getEditorRange, getSelectionOffset, setLastNodeRange, } from "../util/selection";
...
import { getContenteditableElement, getLastBlock, getNextBlock, getTopAloneElement, hasNextSibling, hasPreviousSibling, isNotEditBlock } from "./getBlock";
...
export class WYSIWYG {
    ...
    setEmptyOutline(protyle, element) {
    ...
    bindCommonEvent(protyle) {
        this.element.addEventListener("copy", (event) => {
            ...
            if (selectElements.length > 0) {
                ...
                if (selectElements[0].getAttribute("data-type") === "NodeListItem" &&
                    ...
                    if (isRefText) {
                        const cloneElement = selectElements[0].parentElement.cloneNode(true);
                    ...
                    selectElements.forEach((item, index) => {
                        ...
                        if (isRefText && index === 0) {
                            const cloneElement = topElement.cloneNode(true);
                            const cloneEditElement = getContenteditableElement(cloneElement);
                            if (cloneEditElement) {
        ...
        this.element.addEventListener("mousedown", (event) => {
            ...
            if (!protyle.disabled && target.classList.contains("protyle-action__drag")) {
                ...
                const dragElement = target.previousElementSibling;
            ...
            if (!protyle.disabled && target.classList.contains("table__resize")) {
                ...
                const id = nodeElement.getAttribute("data-node-id");
                const x = event.clientX;
                const colElement = nodeElement.querySelectorAll("table col")[parseInt(target.getAttribute("data-col-index"))];
                // 清空初始化 table 时的最小宽度
                if (colElement.style.minWidth) {
                    colElement.style.width = nodeElement.querySelectorAll("table td, table th")[parseInt(target.getAttribute("data-col-index"))].offsetWidth + "px";
            ...
            let mouseElement;
            let moveCellElement;
            let startFirstElement;
            let endLastElement;
            documentSelf.onmousemove = (moveEvent) => {
                const moveTarget = moveEvent.target;
                // table cell select
                if (!protyle.disabled && tableBlockElement && tableBlockElement.contains(moveTarget) && !hasClosestByClassName(tableBlockElement, "protyle-wysiwyg__embed")) {
                    if ((moveTarget.tagName === "TH" || moveTarget.tagName === "TD") && !moveTarget.isSameNode(target) && (!moveCellElement || !moveCellElement.isSameNode(moveTarget))) {
                        ...
                        Array.from(tableBlockElement.querySelectorAll("th, td")).find((item) => {
                ...
                if (moveEvent.clientY > y) {
                    ...
                    firstBlockElement = protyle.wysiwyg.element.firstElementChild;
                }
                let selectElements = [];
                let currentElement = firstBlockElement;
                let hasJump = false;
                        ...
                        window.siyuan.menus.menu.append(new MenuItem({
                            ...
                            click: () => {
                                if (tableBlockElement) {
                                    ...
                                    let isTHead = false;
                                    let isTBody = false;
                                    tableBlockElement.querySelectorAll("th, td").forEach((item, index) => {
                                    ...
                                    let html = "";
                                    let rowElement = selectCellElements[0].parentElement;
                        ...
                        window.siyuan.menus.menu.append(new MenuItem({
                            ...
                            click: () => {
                                if (tableBlockElement) {
                                    const selectCellElements = [];
                                    const scrollLeft = tableBlockElement.firstElementChild.scrollLeft;
                                    tableBlockElement.querySelectorAll("th, td").forEach((item) => {
                                        if (!item.classList.contains("fn__none") &&
                ...
                if (getSelection().rangeCount > 0) {
                    ...
                    if (range.toString() === "" ||
                        ...
                        if (event.detail === 3) {
                            ...
                            let cursorElement = hasClosestBlock(range.startContainer);
                            if (cursorElement) {
                    ...
                    let endBlockElement;
                    if (mouseUpEvent.detail === 3 && range.endContainer.nodeType !== 3 && range.endContainer.tagName === "DIV" && range.endOffset === 0) {
                        // 三击选中段落块时，rangeEnd 会在下一个块
                        if (range.endContainer.classList.contains("protyle-attr")) {
    ...
    bindEvent(protyle) {
            ...
            if (selectElements.length > 0) {
                ...
                if (headElement && range.toString() === headElement.firstElementChild.textContent) {
                    ...
                    if (headElement.getAttribute("fold") === "1") {
                        isFoldHeading = true;
                        const headCloneElement = headElement.cloneNode(true);
        ...
        this.element.addEventListener("mousewheel", (event) => {
            ...
            if (tableElement) {
                const tableSelectElement = tableElement.querySelector(".table__select");
        ...
        this.element.addEventListener("dblclick", (event) => {
            if (event.target.tagName === "IMG" && !event.target.classList.contains("emoji")) {
                previewImage(event.target.src, protyle.block.rootID);
                return;
            }
        });
        let shiftStartElement;
        this.element.addEventListener("click", (event) => {
            ...
            if (ctrlIsPressed) {
                ...
                if (ctrlElement) {
                    ctrlElement = getTopAloneElement(ctrlElement);
