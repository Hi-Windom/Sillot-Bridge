...
import { focusBlock, focusByRange, focusByWbr, getEditorRange, getSelectionOffset, getSelectionPosition, selectAll, } from "../util/selection";
import { hasClosestBlock, hasClosestByAttribute, hasClosestByClassName, hasClosestByMatchTag, hasTopClosestByAttribute } from "../util/hasClosest";
import { removeBlock } from "./remove";
import { getContenteditableElement, getFirstBlock, getLastBlock, getNextBlock, getPreviousBlock, getTopAloneElement, hasNextSibling, hasPreviousSibling, isNotEditBlock, } from "./getBlock";
...
/// #endif
import { alignImgCenter, alignImgLeft, commonHotkey, downSelect, duplicateBlock, getStartEndElement, goEnd, goHome, upSelect } from "./commonHotkey";
...
export const keydown = (protyle, editorElement) => {
    editorElement.addEventListener("keydown", (event) => {
        ...
        if (!event.altKey && !event.shiftKey && !isCtrl(event) && (event.key === "ArrowDown" || event.key === "ArrowUp")) {
            ...
            if (selectElements.length > 0) {
                ...
                if (event.key === "ArrowDown") {
                    const currentSelectElement = selectElements[selectElements.length - 1];
                    let nextElement = getNextBlock(currentSelectElement);
                    if (nextElement) {
                        if (nextElement.getBoundingClientRect().width === 0) {
                            ...
                            if (foldElement) {
                                nextElement = getNextBlock(foldElement);
                                if (nextElement) {
                                    nextElement = getFirstBlock(nextElement);
                                }
                                else {
                    ...
                    let previousElement = getPreviousBlock(selectElements[0]);
                    if (previousElement) {
                        previousElement = getLastBlock(previousElement);
                        if (previousElement.getBoundingClientRect().width === 0) {
                            // https://github.com/siyuan-note/siyuan/issues/4294
                            const foldElement = hasTopClosestByAttribute(previousElement, "fold", "1");
                            if (foldElement) {
                                previousElement = getFirstBlock(foldElement);
                            }
                            else {
                                previousElement = selectElements[0];
                            }
                        }
        ...
        if (matchHotKey(window.siyuan.config.keymap.editor.general.expandUp.custom, event)) {
            upSelect({
                ...
                cb(selectElements) {
                    const previousElement = selectElements[0].previousElementSibling;
        ...
        if (matchHotKey(window.siyuan.config.keymap.editor.general.expandDown.custom, event)) {
            downSelect({
                ...
                cb(selectElements) {
                    ...
                    const nextElement = selectLastElement.nextElementSibling;
            ...
            upSelect({
                ...
                cb(selectElements) {
                    ...
                    if (startEndElement.startElement.getBoundingClientRect().top >= startEndElement.endElement.getBoundingClientRect().top) {
                        const previousElement = startEndElement.endElement.previousElementSibling;
                        if (previousElement?.getAttribute("data-node-id")) {
                            previousElement.classList.add("protyle-wysiwyg--select");
                            previousElement.setAttribute("select-end", "true");
        ...
        if (matchHotKey("⇧↓", event)) {
            downSelect({
                ...
                cb(selectElements) {
                    const startEndElement = getStartEndElement(selectElements);
                    if (startEndElement.startElement.getBoundingClientRect().top <= startEndElement.endElement.getBoundingClientRect().top) {
                        const nextElement = startEndElement.endElement.nextElementSibling;
                        if (nextElement?.getAttribute("data-node-id")) {
                            nextElement.classList.add("protyle-wysiwyg--select");
                            nextElement.setAttribute("select-end", "true");
                            startEndElement.endElement.removeAttribute("select-end");
        ...
        if (!event.altKey && !event.shiftKey && !isCtrl(event) && !event.isComposing && (event.key.indexOf("Arrow") > -1)) {
            ...
            if (event.key === "ArrowDown" && nodeEditableElement?.textContent.trimRight().substr(position.start).indexOf("\n") === -1 && ((tdElement && !tdElement.parentElement.nextElementSibling && nodeElement.getAttribute("data-type") === "NodeTable" && !getNextBlock(nodeElement)) ||
                ...
                if ((!getPreviousBlock(nodeElement) && // 列表第一个块为嵌入块，第二个块为段落块，上键应选中第一个块 https://ld246.com/article/1652667912155
            ...
            if (event.key === "ArrowDown") {
                ...
                if (foldElement) {
                    // 本身为折叠块
                    let nextElement = getNextBlock(foldElement);
                    if (nextElement) {
                    ...
                    // 下一个块是折叠块
                    const nextFoldElement = getNextBlock(nodeElement);
        ...
        if (!event.altKey && (event.key === "Backspace" || event.key === "Delete")) {
            ...
            if (range.startOffset === 0 &&
                ...
                previousSibling.nodeType !== 3 && previousSibling.outerHTML === "<span>\\</span>" &&
            ...
            if (imgSelectElement) {
                ...
                if (event.key === "Delete") {
                    ...
                    const currentNode = range.startContainer.childNodes[range.startOffset - 1];
                    if (position.start === 0 && (range.startOffset === 0 ||
                    ...
                    if (range.startContainer.nodeType !== 3 &&
                        ...
                        range.startContainer.children[range.startOffset - 1]?.tagName === "TABLE") {
                        nodeElement.classList.add("protyle-wysiwyg--select");
                        removeBlock(protyle, nodeElement, range);
                        event.stopPropagation();
                        event.preventDefault();
        ...
        if (matchHotKey(window.siyuan.config.keymap.editor.general.alignLeft.custom, event)) {
            ...
            if (imgSelectElements.length > 0) {
                ...
                updateBatchTransaction(selectElements, protyle, (e) => {
        ...
        if (!nodeElement.classList.contains("code-block")) {
            const findToolbar = protyle.options.toolbar.find((menuItem) => {
                if (!menuItem.hotkey) {
                    return false;
                }
            ...
            if (range.toString() === "") {
                ...
                selectsElement.forEach(item => {
                    item.querySelectorAll('[contenteditable="true"]').forEach(editItem => {
                        const cloneNode = editItem.cloneNode(true);
        ...
        if (matchHotKey(window.siyuan.config.keymap.editor.general.duplicate.custom, event)) {
            event.preventDefault();
            event.stopPropagation();
            let selectsElement = Array.from(protyle.wysiwyg.element.querySelectorAll(".protyle-wysiwyg--select"));
            if (selectsElement.length === 0) {
