...
import { fixTableRange, focusBlock, focusByRange, focusByWbr, getEditorRange, getSelectionPosition, setFirstNodeRange, setLastNodeRange } from "../util/selection";
...
export class Toolbar {
    ...
    constructor(protyle) {
        ...
        options.toolbar.forEach((menuItem) => {
            const itemElement = this.genItem(protyle, menuItem);
            this.element.appendChild(itemElement);
        });
    }
    render(protyle, range, event) {
        this.range = range;
        ...
        if ((hasImg && noText) ||
            // 拖拽图片到最右侧
            (range.commonAncestorContainer.nodeType !== 3 && range.commonAncestorContainer.classList.contains("img"))) {
            this.element.classList.add("fn__none");
            return;
        }
    ...
    getCurrentType(range = this.range) {
        let types = [];
        ...
        if (startElement.nodeType === 3) {
            ...
            startElement = startElement.childNodes[range.startOffset];
        }
        ...
        if (endElement.nodeType === 3) {
            ...
            endElement = endElement.childNodes[range.endOffset];
        }
    ...
    mergeNode(nodes) {
        for (let i = 0; i < nodes.length; i++) {
            if (nodes[i].nodeType !== 3 && nodes[i].tagName === "WBR") {
                nodes[i].remove();
                i--;
            }
        }
    ...
    setInlineMark(protyle, type, action, textObj) {
        ...
        if (!["DIV", "TD", "TH", "TR"].includes(this.range.endContainer.parentElement.tagName)) {
            if (this.range.endOffset === this.range.endContainer.textContent.length && !nextSibling) {
                nextElement = this.range.endContainer.parentElement.nextSibling;
        ...
        const newNodes = [];
        if (type === "clear" || actionBtn?.classList.contains("protyle-toolbar__item--current") || (action === "range" && rangeTypes.length > 0 && rangeTypes.includes(type) && !textObj)) {
            // 移除
            if (type === "clear") {
            ...
            contents.childNodes.forEach((item, index) => {
        ...
        if (this.range.startContainer.nodeType !== 3 && this.range.startContainer.tagName === "SPAN" &&
            ...
            if (currentNewNode.nodeType !== 3 && nextNewNode && nextNewNode.nodeType !== 3 &&
                ...
                if (currentNewNode.nodeType !== 3 && ["code", "tag", "kbd"].includes(type)) {
                    ...
                    if (!currentNextSibling ||
                        (currentNextSibling && (currentNextSibling.nodeType !== 3 ||
        ...
        if (previousIndex) {
            ...
            if (newNodes[0].nodeType !== 3 && newNodes[0].getAttribute("data-type") === "inline-math") {
                // 数学公式后面处理
            }
            else {
        ...
        if (nextIndex) {
            ...
            if (lastNewNode.nodeType !== 3 && lastNewNode.getAttribute("data-type").indexOf("inline-math") > -1) {
        ...
        if (type === "inline-math") {
            ...
            if (selectText === "") {
                protyle.toolbar.showRender(protyle, newNodes[0], undefined, html);
                return;
            }
        }
        else if (type === "inline-memo") {
            protyle.toolbar.showRender(protyle, newNodes[0], newNodes, html);
            return;
        }
    ...
    showFileAnnotationRef(protyle, refElement) {
...
<div class="fn__flex"><span class="fn__flex-1"></span>
    <button class="b3-button b3-button--cancel">${window.siyuan.languages.remove}</button>
        ...
        const anchorElement = this.subElement.querySelector('[data-type="anchor"]');
    ...
    showRender(protyle, renderElement, updateElements, oldHTML) {
...
- baz`;
        ...
        const pinData = {};
        if (isPin) {
            const textElement = this.subElement.querySelector(".b3-text-field");
...
<textarea spellcheck="false" class="b3-text-field b3-text-field--text fn__block" placeholder="${placeholder}" style="width:${isMobile() ? "100vw" : Math.max(480, renderElement.clientWidth * 0.7) + "px"};max-height:50vh"></textarea></div>`;
    ...
    showCodeLanguage(protyle, languageElement) {
...
<div class="b3-list fn__flex-1 b3-list--background" style="position: relative">${html}</div>
        ...
        inputElement.addEventListener("keydown", (event) => {
            ...
            upDownHint(this.subElement.lastElementChild.lastElementChild, event);
            if (event.key === "Enter") {
                ...
                editElement.textContent = editElement.textContent;
    ...
    showTpl(protyle, nodeElement, range) {
        this.range = range;
        hideElements(["hint"], protyle);
        window.siyuan.menus.menu.remove();
...
<div style="width: 520px;${isMobile() || window.outerWidth < window.outerWidth / 2 + 520 ? "display:none" : ""};overflow: auto;"></div>
    ...
    showWidget(protyle, nodeElement, range) {
        this.range = range;
        hideElements(["hint"], protyle);
        window.siyuan.menus.menu.remove();
...
<div class="b3-list fn__flex-1 b3-list--background" style="position: relative">${html}</div>
    ...
    showAssets(protyle, nodeElement, range) {
        this.range = range;
        hideElements(["hint"], protyle);
        window.siyuan.menus.menu.remove();
