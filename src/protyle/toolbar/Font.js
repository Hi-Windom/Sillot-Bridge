...
const fontMenu = (protyle) => {
    ...
    let textElement = protyle.toolbar.range.cloneContents().querySelector('[data-type~="text"]');
    let fontSize = "16px";
    if (!textElement) {
        textElement = hasClosestByAttribute(protyle.toolbar.range.startContainer, "data-type", "text");
    }
    if (textElement) {
...
<button class="b3-button b3-button--cancel" data-type="clear"><svg><use xlink:href="#iconTrashcan"></use></svg>${window.siyuan.languages.clearFontStyle}</button>`;
    ...
    element.querySelector("select").addEventListener("change", function (event) {
        fontEvent(protyle, "fontSize", event.target.value);
    });
    return element;
};
...
export const setFontStyle = (textElement, textOption) => {
    const setBlockRef = (blockRefOption) => {
...
export const hasSameTextStyle = (currentElement, sideElement, textObj) => {
    if (!textObj) {
        return true;
    }
