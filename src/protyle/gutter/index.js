...
import { transaction, turnsIntoOneTransaction, turnsIntoTransaction, updateBatchTransaction, updateTransaction } from "../wysiwyg/transaction";
...
export class Gutter {
    ...
    constructor(protyle) {
        ...
        this.element.addEventListener("dragstart", (event) => {
            ...
            event.dataTransfer.setData(`${Constants.SIYUAN_DROP_GUTTER}${event.target.getAttribute("data-type")}${Constants.ZWSP}${event.target.getAttribute("data-subtype")}${Constants.ZWSP}${selectIds}`, protyle.wysiwyg.element.innerHTML);
        });
        ...
        this.element.addEventListener("click", (event) => {
            ...
            if (!id) {
                ...
                if (event.altKey) {
                    ...
                    if (foldStatus === "1") {
                        buttonElement.firstElementChild.style.transform = "";
                    }
                    else if (foldStatus === "0") {
                        buttonElement.firstElementChild.style.transform = "rotate(90deg)";
                    }
                }
    ...
    renderMultipleMenu(protyle, selectsElement) {
        // 选中多个连续块的菜单
        let isList = false;
    ...
    renderMenu(protyle, buttonElement) {
        // 单个块的菜单
        ...
        SillotExtSubmennu.push({
            ...
            click() {
                ...
                const initTheme = window.siyuan.config.appearance.mode === 0 ? "vs" : "vs-dark";
                MDDialog({
                    id: "app5",
                    nodeID: id,
                    lang: "markdown",
        ...
        const copyMenu = copySubMenu(id, true, nodeElement).concat([{
                label: window.siyuan.languages.copy,
                accelerator: "⌘C",
                click() {
...
<input style="margin: 4px 0;width: 84px" type="number" step="1" min="148" class="b3-text-field" value="${height ? parseInt(height) : "420"}">`,
                        bind: (element) => {
                            element.querySelector("input").addEventListener("change", (event) => {
                                const newHeight = (event.target.value || "420") + "px";
                                nodeElement.style.height = newHeight;
                                nodeElement.firstElementChild.nextElementSibling.style.height = newHeight;
...
<input type="checkbox" class="b3-switch fn__flex-center"${nodeElement.getAttribute("custom-heading-mode") === "1" ? " checked" : ""}></div>`,
                ...
                click() {
                    fetchPost("/api/block/getHeadingChildrenDOM", { id }, (response) => {
                        ...
                        fetchPost("/api/block/getHeadingDeleteTransaction", {
                            ...
                            response.data.doOperations.forEach((operation) => {
                                protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${operation.id}"]`).forEach((itemElement) => {
                                    itemElement.remove();
                                });
                            });
        ...
        window.siyuan.menus.menu.append(new MenuItem({ type: "separator" }).element);
        const SSSubmennu = []; // https://github.com/Hi-Windom/Sillot/issues/175
    ...
    genHeadingTransform(protyle, id, level) {
        return {
    ...
    genAlign(nodeElements, protyle) {
        window.siyuan.menus.menu.append(new MenuItem({
            ...
            submenu: [{
                    ...
                    click: () => {
                        this.genClick(nodeElements, protyle, (e) => {
    ...
    genBGStyle(nodeElements, protyle) {
        const styles = [];
    ...
    genWidths(nodeElements, protyle) {
        const styles = [];
        ...
        if (nodeElements.length === 1) {
            const widthStyle = nodeElements[0].style.width;
    ...
    genFontStyle(nodeElements, protyle) {
        const styles = [];
    ...
    genCardStyle(nodeElements, protyle) {
        const styles = [];
    ...
    render(protyle, element, wysiwyg) {
        ...
        if (left < this.element.parentElement.getBoundingClientRect().left) {
            ...
            Array.from(this.element.children).reverse().forEach((item, index) => {
                if (index !== 0) {
                    item.firstElementChild.style.height = "14px";
                }
                html += item.outerHTML;
            });
            this.element.innerHTML = html;
