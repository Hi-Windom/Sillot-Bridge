...
export const getColIndex = (cellElement) => {
...
// 光标设置到前一个表格中
const goPreviousCell = (cellElement, range, isSelected = true) => {
    let previousElement = cellElement.previousElementSibling;
    if (!previousElement) {
...
export const setTableAlign = (protyle, cellElements, nodeElement, type, range) => {
...
export const insertRow = (protyle, range, cellElement, nodeElement) => {
    ...
    if (cellElement.tagName === "TH") {
        ...
        if (tbodyElement) {
            ...
            newRowElememt = tbodyElement.firstElementChild;
        }
        else {
            cellElement.parentElement.parentElement.insertAdjacentHTML("afterend", `<tbody><tr>${rowHTML}</tr></tbody>`);
            newRowElememt = cellElement.parentElement.parentElement.nextElementSibling.firstElementChild;
        }
    }
    else {
        cellElement.parentElement.insertAdjacentHTML("afterend", `<tr>${rowHTML}</tr>`);
        newRowElememt = cellElement.parentElement.nextElementSibling;
    }
...
export const insertRowAbove = (protyle, range, cellElement, nodeElement) => {
    const wbrElement = document.createElement("wbr");
    range.insertNode(wbrElement);
    const html = nodeElement.outerHTML;
    wbrElement.remove();
    ...
    let hasNone = false;
    for (let m = 0; m < cellElement.parentElement.childElementCount; m++) {
        const currentCellElement = cellElement.parentElement.children[m];
    ...
    if (hasNone) {
        ...
        while (previousTrElement) {
            rowCount++;
            Array.from(previousTrElement.children).forEach((cell) => {
    ...
    if (cellElement.parentElement.parentElement.tagName === "THEAD" && !cellElement.parentElement.previousElementSibling) {
        ...
        newRowElememt = cellElement.parentElement.previousElementSibling;
    }
    range.selectNodeContents(newRowElememt.cells[getColIndex(cellElement)]);
    range.collapse(true);
    focusByRange(range);
...
export const insertColumn = (protyle, nodeElement, cellElement, type, range) => {
    const wbrElement = document.createElement("wbr");
    range.insertNode(wbrElement);
    const html = nodeElement.outerHTML;
    wbrElement.remove();
...
export const deleteRow = (protyle, range, cellElement, nodeElement) => {
    if (cellElement.parentElement.parentElement.tagName !== "THEAD") {
        ...
        let previousTrElement = tbodyElement.previousElementSibling.lastElementChild;
        if (cellElement.parentElement.previousElementSibling) {
            previousTrElement = cellElement.parentElement.previousElementSibling;
        }
...
export const deleteColumn = (protyle, range, nodeElement, cellElement) => {
    const wbrElement = document.createElement("wbr");
    range.insertNode(wbrElement);
    const html = nodeElement.outerHTML;
    wbrElement.remove();
    const index = getColIndex(cellElement);
    const sideCellElement = (cellElement.previousElementSibling || cellElement.nextElementSibling);
    if (sideCellElement) {
...
export const moveRowToUp = (protyle, range, cellElement, nodeElement) => {
...
export const moveRowToDown = (protyle, range, cellElement, nodeElement) => {
    const rowElement = cellElement.parentElement;
...
export const moveColumnToLeft = (protyle, range, cellElement, nodeElement) => {
...
export const moveColumnToRight = (protyle, range, cellElement, nodeElement) => {
...
export const fixTable = (protyle, event, range) => {
    ...
    if (!isCtrl(event) && !event.shiftKey && !event.altKey && event.key === "Enter") {
        ...
        let nextElement = trElement.nextElementSibling;
        if (!nextElement) {
    ...
    if (event.key === "ArrowUp" && !isCtrl(event) && !event.shiftKey && !event.altKey) {
        ...
        if (startContainer.nodeType !== 3 && (startContainer.tagName === "TH" || startContainer.tagName === "TD")) {
            previousBrElement = startContainer.childNodes[Math.max(0, range.startOffset - 1)]?.previousElementSibling;
        }
        ...
        const trElement = cellElement.parentElement;
        let previousElement = trElement.previousElementSibling;
        if (!previousElement) {
            previousElement = trElement.parentElement.previousElementSibling.lastElementChild;
        }
    ...
    if (event.key === "ArrowDown" && !isCtrl(event) && !event.shiftKey && !event.altKey) {
        ...
        if (endContainer.nodeType !== 3 && (endContainer.tagName === "TH" || endContainer.tagName === "TD")) {
            nextBrElement = endContainer.childNodes[Math.max(0, range.endOffset - 1)]?.nextElementSibling;
        }
    ...
    Array.from(cellElement.parentElement.children).forEach((item) => {
        if (item.colSpan > 1) {
            hasColSpan = true;
        }
        if (item.rowSpan > 1) {
    ...
    if (previousRowElement) {
        ...
        Array.from(previousRowElement.children).forEach((item) => {
            if (item.colSpan > 1) {
    ...
    if (nextRowElement) {
        ...
        Array.from(nextRowElement.children).forEach((item) => {
            if (item.colSpan > 1) {
                nextHasColSpan = true;
            }
            if (item.rowSpan > 1) {
