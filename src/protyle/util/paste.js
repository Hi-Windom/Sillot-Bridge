...
const filterClipboardHint = (protyle, textPlain) => {
    let needRender = true;
...
export const pasteAsPlainText = async (protyle) => {
    /// #if !BROWSER
    let localFiles = [];
...
export const pasteText = (protyle, textPlain, nodeElement) => {
...
export const paste = async (protyle, event) => {
    ...
    if (nodeElement.getAttribute("data-type") === "NodeCodeBlock") {
        ...
        if (textHTML.replace("<!--StartFragment--><!--EndFragment-->", "").trim() !== "") {
            ...
            if (files && files.length === 1 && (textHTML.startsWith("<img") || // 浏览器上复制单个图片
