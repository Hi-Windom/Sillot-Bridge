...
const moveToNew = (protyle, sourceElements, targetElement, newSourceElement, isSameDoc, isBottom, isCopy) => {
    ...
    sourceElements.reverse().forEach((item, index) => {
        ...
        if (isCopy) {
            const copyElement = item.cloneNode(true);
...
const moveTo = async (protyle, sourceElements, targetElement, isSameDoc, position, isCopy) => {
    let topSourceElement;
    const doOperations = [];
    const undoOperations = [];
    ...
    sourceElements.reverse().forEach((item, index) => {
        ...
        if (isCopy) {
            copyElement = item.cloneNode(true);
            copyElement.setAttribute("data-node-id", copyId);
            copyElement.querySelectorAll("[data-node-id]").forEach((e) => {
                const newId = Lute.NewNodeID();
                e.setAttribute("data-node-id", newId);
            ...
            doOperations.push({
                ...
                id: copyId,
                data: copyElement.outerHTML,
                previousID: position === "afterend" ? targetId : copyElement.previousElementSibling?.getAttribute("data-node-id"),
            ...
            doOperations.push({
                action: "move",
                id,
                previousID: position === "afterend" ? targetId : item.previousElementSibling?.getAttribute("data-node-id"),
    ...
    for (let j = 0; j < foldHeadingIds.length; j++) {
        ...
        headingIds.data.reverse().forEach((headingId) => {
            undoOperations.push({
                action: "move",
                id: headingId,
...
const dragSb = async (protyle, sourceElements, targetElement, isBottom, direct, isCopy) => {
    ...
    if (!isCopy && oldSourceParentElement && oldSourceParentElement.classList.contains("sb") && oldSourceParentElement.childElementCount === 2) {
        ...
        if (protyleElement && !protyleElement.classList.contains("block__edit")) {
            const editor = getInstanceById(protyleElement.getAttribute("data-id"));
...
const dragSame = async (protyle, sourceElements, targetElement, isBottom, isCopy) => {
    const isSameDoc = protyle.element.contains(sourceElements[0]);
    const doOperations = [];
    const undoOperations = [];
    let newSourceElement;
...
export const dropEvent = (protyle, editorElement) => {
    editorElement.addEventListener("dragstart", (event) => {
        ...
        if (target.classList?.contains("protyle-action")) {
            if (hasClosestByClassName(target, "protyle-wysiwyg__embed")) {
                ...
                event.dataTransfer.setData(`${Constants.SIYUAN_DROP_GUTTER}NodeListItem${Constants.ZWSP}${target.parentElement.getAttribute("data-subtype")}${Constants.ZWSP}${[target.parentElement.getAttribute("data-node-id")]}`, protyle.wysiwyg.element.innerHTML);
            }
            return;
        }
        // 选中编辑器中的文字进行拖拽
    ...
    editorElement.addEventListener("dragover", (event) => {
        ...
        if (gutterType) {
            ...
            // 排除自己及子孙
            const gutterTypes = gutterType.replace(Constants.SIYUAN_DROP_GUTTER, "").split(Constants.ZWSP);
            const isSelf = gutterTypes[2].split(",").find((item) => {
