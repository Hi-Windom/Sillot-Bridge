...
export const onGet = (data, protyle, action = [], scrollAttr, renderTitle = false) => {
...
const setHTML = (options, protyle) => {
    ...
    if (options.action.includes(Constants.CB_GET_APPEND)) {
        ...
        if (!protyle.wysiwyg.element.querySelector(".protyle-wysiwyg--select") && !protyle.scroll.keepLazyLoad && protyle.contentElement.scrollHeight > REMOVED_OVER_HEIGHT) {
            let removeElement = protyle.wysiwyg.element.firstElementChild;
            ...
            while (protyle.wysiwyg.element.childElementCount > 2 && removeElements && !protyle.wysiwyg.element.lastElementChild.isSameNode(removeElement)) {
                ...
                removeElement = removeElement.nextElementSibling;
            }
        ...
        const lastElement = protyle.wysiwyg.element.firstElementChild;
    ...
    if (options.action.includes(Constants.CB_GET_HL) && !options.unScroll) {
        ...
        let focusElement;
        Array.from(protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${protyle.block.id}"]`)).find((item) => {
...
export const disabledForeverProtyle = (protyle) => {
    disabledProtyle(protyle);
...
/** 禁用编辑器 */
export const disabledProtyle = (protyle) => {
    window.siyuan.menus.menu.remove();
    hideElements(["gutter", "toolbar", "select", "hint", "util"], protyle);
    protyle.disabled = true;
    if (protyle.title) {
        const titleElement = protyle.title.element.querySelector(".protyle-title__input");
...
/** 解除编辑器禁用 */
export const enableProtyle = (protyle) => {
