...
export class Preview {
    ...
    constructor(protyle) {
        ...
        actionElement.addEventListener(getEventName(), (event) => {
            const btn = hasClosestByTag(event.target, "BUTTON");
            if (!btn) {
                return;
            }
            const type = btn.getAttribute("data-type");
            const actionCustom = actions.find((w) => w?.key === type);
            if (actionCustom) {
                actionCustom.click(type);
                return;
            }
            if ((type === "mp-wechat" || type === "zhihu" || type === "yuque")) {
                this.copyToX(this.element.lastElementChild.cloneNode(true), protyle, type);
                return;
            }
            if (type === "desktop") {
    ...
    copyToX(copyElement, protyle, type) {
        // fix math render
        if (type === "mp-wechat") {
    ...
    processZHBlockquote(element, elements) {
