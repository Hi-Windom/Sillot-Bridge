...
export const blockRender = (protyle, element, top) => {
    let blockElements = [];
    ...
    <span class="protyle-icon protyle-action__menu protyle-icon--last"><svg><use xlink:href="#iconMore"></use></svg></span>
        ...
        let breadcrumb = item.getAttribute("breadcrumb");
        if (breadcrumb) {
        ...
        fetchPost("/api/search/searchEmbedBlock", {
            ...
            let html = "";
            response.data.blocks.forEach((blocksItem) => {
                let breadcrumbHTML = "";
