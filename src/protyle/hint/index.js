...
import { focusBlock, focusByRange, focusByWbr, getEditorRange, getSelectionOffset, getSelectionPosition } from "../util/selection";
...
export class Hint {
    ...
    constructor(protyle) {
        ...
        this.element.addEventListener("click", (event) => {
            ...
            if (typeElement) {
                const titleElement = emojisContentElement.querySelector(`[data-type="${typeElement.getAttribute("data-type")}"]`);
                if (titleElement) {
...
${unicode2Emoji(emoji.unicode, true)}</button>`;
    ...
    render(protyle) {
        ...
        if (protyle.toolbar.range.startContainer.nodeType === 3 && protyle.toolbar.range.startContainer.textContent === "") {
            const lastSibling = hasPreviousSibling(protyle.toolbar.range.startContainer);
    ...
    bindUploadEvent(protyle, element) {
    ...
    getHTMLByData(data, hasSearch = false) {
        let hintsHTML = "";
        if (hasSearch) {
    ...
    genHTML(data, protyle, hide = false, hasSearch = false) {
        if (data.length === 0) {
        ...
        if (hasSearch) {
            const searchElement = this.element.querySelector("input.b3-text-field");
            ...
            searchElement.addEventListener("compositionend", (event) => {
                event.stopPropagation();
                this.genSearchHTML(protyle, searchElement, nodeElement, oldValue);
            });
        }
    }
    genSearchHTML(protyle, searchElement, nodeElement, oldValue) {
...
<div class="fn__flex${value ? " fn__none" : ""}">
    ...
    fill(value, protyle) {
        ...
        let nodeElement = hasClosestBlock(protyle.toolbar.range.startContainer);
        if (!nodeElement) {
            return;
        }
        ...
        if (Constants.BLOCK_HINT_KEYS.includes(this.splitChar) && endSplit && range.startContainer.nodeType === 3
            && range.startContainer.wholeText.indexOf(endSplit) > -1) {
            let matchEndChar = 0;
...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
                ...
                if (value === "![]()") { // https://github.com/siyuan-note/siyuan/issues/4586 1
                    ...
                    updateTransaction(protyle, id, nodeElement.outerHTML, html);
                    let imgElement = range.startContainer.childNodes[range.startOffset - 1] || range.startContainer;
                    if (imgElement && imgElement.nodeType !== 3 && imgElement.classList.contains("img")) {
                        // 已经找到图片
                    }
                    else if (imgElement.previousSibling?.nodeType !== 3 && imgElement.previousSibling.classList.contains("img")) {
                        // https://github.com/siyuan-note/siyuan/issues/7540
                        imgElement = imgElement.previousSibling;
                    }
                    else {
                        Array.from(nodeElement.querySelectorAll(".img")).find((item) => {
                    ...
                    if (nodeElement.getAttribute("data-type") === "NodeTable") {
                        nodeElement.querySelectorAll("colgroup col").forEach((item) => {
        ...
        if (event.key === "Enter") {
            if (isEmojiPanel) {
                ...
                if (mark === Constants.ZWSP + 3) {
                    this.element.querySelector(".b3-list-item--focus input").click();
                }
                else {
        ...
        if (isEmojiPanel) {
            ...
            if (event.key === "ArrowLeft" || event.key === "ArrowUp") {
                if (currentElement.previousElementSibling) {
                    ...
                    newCurrentElement = currentElement.previousElementSibling;
                }
                else if (currentElement.parentElement.previousElementSibling?.previousElementSibling) {
                    currentElement.classList.remove("emojis__item--current");
                    newCurrentElement = currentElement.parentElement.previousElementSibling.previousElementSibling.lastElementChild;
                }
            }
            else if (event.key === "ArrowRight" || event.key === "ArrowDown") {
                if (currentElement.nextElementSibling) {
                    currentElement.classList.remove("emojis__item--current");
                    newCurrentElement = currentElement.nextElementSibling;
                }
                else if (currentElement.parentElement.nextElementSibling?.nextElementSibling) {
                    currentElement.classList.remove("emojis__item--current");
                    newCurrentElement = currentElement.parentElement.nextElementSibling.nextElementSibling.firstElementChild;
                }
            }
    ...
    fixImageCursor(range) {
        ...
        if (previous && previous.nodeType !== 3 && previous.classList.contains("img")) {
    ...
    getKey(currentLineValue, extend) {
        this.lastIndex = -1;
        this.splitChar = "";
        extend.forEach((item) => {
