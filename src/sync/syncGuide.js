...
export const addCloudName = (cloudPanelElement) => {
...
export const bindSyncCloudListEvent = (cloudPanelElement) => {
...
export const getSyncCloudList = (cloudPanelElement, reload = false, cb) => {
    ...
    fetchPost("/api/sync/listCloudSyncDir", {}, (response) => {
        ...
        if (response.code === 1) {
            ...
            response.data.syncDirs.forEach((item) => {
...
export const syncGuide = (element) => {
...
<div class="b3-dialog__action">
    ...
    btnsElement[1].addEventListener("click", () => {
        const uploadElement = manualDialog.element.querySelector("input[name=upload]:checked");
        if (!uploadElement) {
