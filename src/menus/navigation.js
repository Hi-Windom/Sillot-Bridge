...
const initMultiMenu = (selectItemElements) => {
    ...
    window.siyuan.menus.menu.append(movePathToMenu(getTopPaths(Array.from(selectItemElements))));
...
export const initNavigationMenu = (liElement) => {
    ...
    if (!window.siyuan.config.readonly) {
        ...
        sortMenu("notebook", parseInt(liElement.parentElement.getAttribute("data-sortmode")), (sort) => {
            fetchPost("/api/notebook/setNotebookConf", {
                ...
                /// #else
                files = getDockByType("file").data["file"];
                /// #endif
...
export const initFileMenu = (notebookId, pathString, liElement) => {
    window.siyuan.menus.menu.remove();
    const fileElement = hasClosestByTag(liElement, "DIV");
    if (!fileElement) {
        return window.siyuan.menus.menu;
    ...
    if (!window.siyuan.config.readonly) {
        ...
        window.siyuan.menus.menu.append(new MenuItem({
            ...
            type: "submenu",
            icon: "iconCopy",
            submenu: copySubMenu(id, false).concat([{
        ...
        window.siyuan.menus.menu.append(movePathToMenu(getTopPaths(Array.from(fileElement.querySelectorAll(".b3-list-item--focus")))));
        window.siyuan.menus.menu.append(new MenuItem({
            icon: "iconTrashcan",
            label: window.siyuan.languages.delete,
            accelerator: "⌦",
...
const genImportMenu = (notebookId, pathString) => {
    if (!window.siyuan.config.readonly) {
        /// #if !BROWSER
        const importstdmd = (label, isDoc) => {
            return {
        ...
        window.siyuan.menus.menu.append(new MenuItem({
            ...
            submenu: [{
                    ...
                    bind: (element) => {
                        element.querySelector(".b3-form__upload").addEventListener("change", (event) => {
...
export const sortMenu = (type, sortMode, clickEvent) => {
    const submenu = [{
