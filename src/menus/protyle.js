...
import { deleteColumn, deleteRow, getColIndex, insertColumn, insertRow, insertRowAbove, moveColumnToLeft, moveColumnToRight, moveRowToDown, moveRowToUp, setTableAlign } from "../protyle/util/table";
...
export const refMenu = (protyle, element) => {
...
export const contentMenu = (protyle, nodeElement) => {
    const range = getEditorRange(nodeElement);
    window.siyuan.menus.menu.remove();
    if (range.toString() !== "" || range.cloneContents().childNodes[0]?.classList.contains("emoji")) {
        window.siyuan.menus.menu.append(new MenuItem({
            icon: "iconCopy",
            accelerator: "⌘C",
            label: window.siyuan.languages.copy,
    ...
    if (nodeElement.classList.contains("table")) {
        ...
        if (cellElement) {
            window.siyuan.menus.menu.append(new MenuItem({
                ...
                submenu: tableMenu(protyle, nodeElement, cellElement, range)
            }).element);
        }
    }
};
export const zoomOut = (protyle, id, focusId, isPushBack = true, callback) => {
...
export const imgMenu = (protyle, range, assetElement, position) => {
    window.siyuan.menus.menu.remove();
    ...
    const titleElement = assetElement.querySelector(".protyle-action__title");
...
export const linkMenu = (protyle, linkElement, focusText = false) => {
    window.siyuan.menus.menu.remove();
...
export const tagMenu = (protyle, tagElement) => {
    window.siyuan.menus.menu.remove();
...
const genImageWidthMenu = (label, assetElement, imgElement, protyle, id, nodeElement, html) => {
    return {
        label,
        click() {
            nodeElement.setAttribute("updated", format(new Date(), 'yyyyMMddHHmmss'));
...
export const iframeMenu = (protyle, nodeElement) => {
    const id = nodeElement.getAttribute("data-node-id");
    ...
    if (iframeSrc) {
        ...
        return subMenus.concat(openMenu(iframeSrc, true, false));
    }
    return subMenus;
};
export const videoMenu = (protyle, nodeElement, type) => {
    const id = nodeElement.getAttribute("data-node-id");
    ...
    const subMenus = [{
            ...
            bind(element) {
                element.querySelector("textarea").addEventListener("change", (event) => {
                    videoElement.setAttribute("src", event.target.value.replace(/\n|\r\n|\r|\u2028|\u2029/g, ""));
                    updateTransaction(protyle, id, nodeElement.outerHTML, html);
                    html = nodeElement.outerHTML;
                    event.stopPropagation();
                });
    ...
    if (VideoSrc) {
        return subMenus.concat(openMenu(VideoSrc, true, false));
    }
    return subMenus;
};
export const tableMenu = (protyle, nodeElement, cellElement, range) => {
    const menus = [];
    ...
    if (cellElement.rowSpan > 1 || cellElement.colSpan > 1) {
        menus.push({
            ...
            click: () => {
                ...
                let currentRowElement = cellElement.parentElement;
                const orgColSpan = cellElement.colSpan;
                while (rowSpan > 0 && currentRowElement) {
                    let currentCellElement = currentRowElement.children[colIndex];
                    ...
                    while (colSpan > 0 && currentCellElement) {
                        ...
                        currentCellElement = currentCellElement.nextElementSibling;
                        colSpan--;
                    }
                ...
                if (cellElement.tagName === "TH") {
                    let prueTrElement;
                    Array.from(nodeElement.querySelectorAll("thead tr")).find((item) => {
                        prueTrElement = item;
                        Array.from(item.children).forEach((cellElement) => {
...
export const setFold = (protyle, nodeElement, isOpen, isRemove) => {
    ...
    if (nodeElement.getAttribute("fold") === "1") {
        ...
        nodeElement.querySelectorAll(".protyle-linenumber").forEach((item) => {
