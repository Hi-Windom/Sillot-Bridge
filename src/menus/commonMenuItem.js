...
const bindAttrInput = (inputElement, confirmElement) => {
...
export const openWechatNotify = (nodeElement) => {
...
export const openFileWechatNotify = (protyle) => {
...
export const openFileAttr = (attrs, id, focusName = "bookmark") => {
    let customHTML = "";
    let notifyHTML = "";
...
<div class="b3-dialog__action">
    ...
    dialog.element.querySelector('.b3-text-field[data-name="bookmark"]').value = attrs.bookmark || "";
    dialog.element.querySelector('.b3-text-field[data-name="name"]').value = attrs.name || "";
    dialog.element.querySelector('.b3-text-field[data-name="alias"]').value = attrs.alias || "";
    const removeAttrs = [];
    ...
    <span data-action="remove" class="block__icon fn__flex-center" style="opacity: 1;"><svg><use xlink:href="#iconMin"></use></svg></span>
        ...
        const inputElements = dialog.element.querySelectorAll(".b3-text-field");
    ...
    btnsElement[2].addEventListener("click", () => {
        ...
        let errorTip = "";
        const attrsResult = {};
        dialog.element.querySelectorAll(".fn__flex-1").forEach((item) => {
            const name = item.getAttribute("data-name") || ("custom-" + item.previousElementSibling.previousElementSibling.value);
            if (item.value.trim()) {
...
export const openAttr = (nodeElement, protyle, focusName = "bookmark") => {
...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
        ...
        dialog.element.querySelector('.b3-text-field[data-name="bookmark"]').value = response.data.bookmark || "";
        dialog.element.querySelector('.b3-text-field[data-name="name"]').value = response.data.name || "";
        dialog.element.querySelector('.b3-text-field[data-name="alias"]').value = response.data.alias || "";
        const removeAttrs = [];
        dialog.element.addEventListener("click", (event) => {
            const target = event.target;
            const actionElement = hasClosestByClassName(target, "block__icon");
...
export const copySubMenu = (id, accelerator = true, focusElement) => {
    return [
        {
            icon: "iconRef",
...
export const openMenu = (src, onlyMenu, showAccelerator) => {
    const submenu = [];
    if (isLocalPath(src)) {
...
export const movePathToMenu = (paths) => {
    return new MenuItem({
