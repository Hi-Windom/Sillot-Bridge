...
export function exAce() {
    ...
    window.__.ace.createEditor = async (id, initMode, initCode) => {
        let editor = false;
        return await Swal.fire({
            title: "",
            html: /*html*/ `<div class="ace-container" style="height: calc(100% - 10px);">
        ...
        <textarea id="aceEditorTextareaOriginElement"></textarea>
            ...
            width: "100vw",
            didOpen: (thisE) => {
                const _t = thisE.querySelector(".ace-container textarea");
                if (_t) {
                    editor = ace.edit("aceEditorTextareaOriginElement"); // 放在最前面
                    window.__.ace.editor = editor; // 曲线救国
                    const _swal2_container = thisE.parentElement; // div.swal2-container
                    const _HTML_c = thisE.querySelector("#swal2-html-container");
                    const _ace_c = _HTML_c.querySelector(".ace-container");
                    const _ace_e = _ace_c.querySelector(".ace_editor");
                    const _rule_line = thisE.querySelector(".ace_print-margin");
                    const _s_fontsize = _ace_c.querySelector("#ace_fontsizeSelector");
                    const _s_mode = _ace_c.querySelector("#ace_modeSelector");
                    const _s_theme = _ace_c.querySelector("#ace_themeSelector");
                    ...
                    let _s_mode_options = "";
                    window._.forEach(window.__.ace.modes, (value, key) => {
                    ...
                    const _mode = window.__.ace.modes[initMode];
                    if (_mode) {
