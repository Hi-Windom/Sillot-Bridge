    ...
    providerPanelElement.querySelectorAll(".b3-text-field, .b3-select").forEach(item => {
        item.addEventListener("blur", () => {
            if (window.siyuan.config.sync.provider === 2) {
                let timeout = parseInt(providerPanelElement.querySelector("#timeout").value, 10);
                if (7 > timeout) {
                    if (1 > timeout) {
                        timeout = 30;
                    }
                ...
                providerPanelElement.querySelector("#timeout").value = timeout.toString();
                const s3 = {
                    endpoint: providerPanelElement.querySelector("#endpoint").value,
                    accessKey: providerPanelElement.querySelector("#accessKey").value,
                    secretKey: providerPanelElement.querySelector("#secretKey").value,
                    bucket: providerPanelElement.querySelector("#bucket").value,
                    pathStyle: providerPanelElement.querySelector("#pathStyle").value === "true",
                    region: providerPanelElement.querySelector("#region").value,
                    skipTlsVerify: providerPanelElement.querySelector("#s3SkipTlsVerify").value === "true",
                    timeout: timeout,
                };
                ...
                const webdav = {
                    endpoint: providerPanelElement.querySelector("#endpoint").value,
                    username: providerPanelElement.querySelector("#username").value,
                    password: providerPanelElement.querySelector("#password").value,
                    skipTlsVerify: providerPanelElement.querySelector("#webdavSkipTlsVerify").value === "true",
                    timeout: timeout,
                };
...
<div class="b3-label fn__flex">
    ...
    bindEvent: () => {
        bindProviderEvent();
        const switchElement = repos.element.querySelector("#reposCloudSyncSwitch");
        ...
        const switchConflictElement = repos.element.querySelector("#generateConflictDoc");
        ...
        const syncModeElement = repos.element.querySelector("#syncMode");
        ...
        const syncProviderElement = repos.element.querySelector("#syncProvider");
