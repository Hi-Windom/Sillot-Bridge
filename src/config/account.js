...
<div class="config-account__center config-account__center--text${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}">
    ...
    bindEvent: (element) => {
        ...
        const agreeLoginElement = element.querySelector("#agreeLogin");
        ...
        if (!userNameElement) {
            ...
            element.querySelectorAll("input[type='checkbox']").forEach(item => {
                item.addEventListener("change", () => {
                    fetchPost("/api/setting/setAccount", {
                        displayTitle: element.querySelector("#displayTitle").checked,
                        displayVIP: element.querySelector("#displayVIP").checked,
                    }, (response) => {
            ...
            activationCodeElement.addEventListener("click", () => {
                const activationCodeInput = activationCodeElement.previousElementSibling;
                fetchPost("/api/account/checkActivationcode", { data: activationCodeInput.value }, (response) => {
                    ...
                    confirmDialog(window.siyuan.languages.activationCode, response.msg, () => {
                        if (response.code === 0) {
                            fetchPost("/api/account/useActivationcode", { data: activationCodeElement.previousElementSibling.value }, () => {
        ...
        const userPasswordElement = element.querySelector("#userPassword");
        const captchaImgElement = element.querySelector("#captchaImg");
        const captchaElement = element.querySelector("#captcha");
        const twofactorAuthCodeElement = element.querySelector("#twofactorAuthCode");
        const loginBtnElement = element.querySelector("#login");
        const login2BtnElement = element.querySelector("#login2");
    ...
    _afterLogin(userResponse, element) {
