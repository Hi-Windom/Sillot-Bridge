...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
        ...
        response.data.snippets.forEach((item) => {
            const nameElement = dialog.element.querySelector(`[data-id="${item.id}"] input`);
            nameElement.value = item.name;
            const contentElement = dialog.element.querySelector(`[data-id="${item.id}"] textarea`);
