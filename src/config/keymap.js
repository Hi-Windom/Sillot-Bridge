...
<div class="b3-label file-tree config-keymap" id="keymapList">
    ...
    _search(value, keymapString) {
        keymap.element.querySelectorAll("#keymapList .b3-list-item--hide-action > .b3-list-item__text").forEach(item => {
            ...
            if (keymapString === "" || item.nextElementSibling.nextElementSibling.value.indexOf(updateHotkeyTip(keymapString)) > -1) {
                matchedKeymap = true;
            }
    ...
    bindEvent() {
        ...
        const searchElement = keymap.element.querySelector("#keymapInput");
        const searchKeymapElement = keymap.element.querySelector("#searchByKey");
        ...
        searchKeymapElement.addEventListener("keydown", function (event) {
        ...
        keymapListElement.addEventListener("click", (event) => {
            ...
            while (target && !target.isEqualNode(keymapListElement)) {
                if (target.classList?.contains("b3-tooltips")) {
                    ...
                    const inputElement = target.parentElement.querySelector(".b3-text-field");
                    if (type === "reset") {
