...
<label class="fn__flex b3-label">
    ...
    _send: () => {
        const themeLight = appearance.element.querySelector("#themeLight").value;
        const themeDark = appearance.element.querySelector("#themeDark").value;
        const modeElementValue = parseInt(appearance.element.querySelector("#mode").value);
        fetchPost("/api/setting/setAppearance", {
            icon: appearance.element.querySelector("#icon").value,
            mode: modeElementValue === 2 ? window.siyuan.config.appearance.mode : modeElementValue,
            modeOS: modeElementValue === 2,
            codeBlockThemeDark: appearance.element.querySelector("#codeBlockThemeDark").value,
            codeBlockThemeLight: appearance.element.querySelector("#codeBlockThemeLight").value,
            themeDark,
            themeLight,
            ...
            lang: appearance.element.querySelector("#lang").value,
            customCSS: appearance.element.querySelector("#appearanceCustom").checked,
            closeButtonBehavior: appearance.element.querySelector("#closeButtonBehavior").checked ? 1 : 0,
            nativeEmoji: appearance.element.querySelector("#nativeEmoji").checked,
            hideStatusBar: appearance.element.querySelector("#hideStatusBar").checked,
        }, response => {
            if (window.siyuan.config.appearance.themeJS) {
                if (!response.data.modeOS && (response.data.mode !== window.siyuan.config.appearance.mode ||
                ...
                if (response.data.modeOS && ((response.data.mode === 1 && OSTheme === "light") || (response.data.mode === 0 && OSTheme === "dark"))) {
                    exportLayout(true);
                    return;
                }
            }
    ...
    onSetappearance(data) {
        ...
        if (appearance.element) {
            ...
            const themeLightElement = appearance.element.querySelector("#themeLight");
            ...
            const themeDarkElement = appearance.element.querySelector("#themeDark");
            if (themeDarkElement) {
            ...
            const iconElement = appearance.element.querySelector("#icon");
            if (iconElement) {
