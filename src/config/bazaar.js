...
<div id="configBazaarReadme" class="config-bazaar__readme"></div>
    ...
    _genCardHTML(item, bazaarType) {
        let hide = false;
        let themeMode = "";
        if (bazaarType === "themes") {
            const themeValue = bazaar.element.querySelector("#bazaarSelect").value;
    ...
    _renderReadme(cardElement, bazaarType) {
        const dataObj = JSON.parse(cardElement.getAttribute("data-obj"));
        let data;
        (dataObj.downloaded ? bazaar._data.downloaded : bazaar._data[bazaarType]).find((item) => {
            if (item.repoURL === dataObj.repoURL) {
                data = item;
                return true;
            }
        });
        const readmeElement = bazaar.element.querySelector("#configBazaarReadme");
...
<div class="item__main">
    ...
    bindEvent() {
            ...
            while (target && !target.isEqualNode(bazaar.element)) {
                ...
                if (type === "open") {
                    ...
                    if (target.classList.contains("b3-button--outline")) {
                        ...
                        this._genMyHTML(type.replace("my", "").toLowerCase() + "s");
                    }
                    event.preventDefault();
                    event.stopPropagation();
                    break;
                    ...
                    if (!target.classList.contains("b3-button--progress")) {
                        const dataObj = JSON.parse(target.parentElement.parentElement.getAttribute("data-obj"));
                        const bazaarType = dataObj.bazaarType;
                    ...
                    if (!target.classList.contains("b3-button--progress")) {
                        confirmDialog(window.siyuan.languages.update, window.siyuan.languages.exportTplTip, () => {
                            ...
                            fetchPost(url, {
                                ...
                                if (bazaarType === "themes" && ((window.siyuan.config.appearance.mode === 0 && window.siyuan.config.appearance.themeLight === dataObj.name) ||
                                    (window.siyuan.config.appearance.mode === 1 && window.siyuan.config.appearance.themeDark === dataObj.name))) {
                                    if (window.siyuan.config.appearance.themeJS) {
                                        exportLayout(true);
                                    }
                                    else {
                                        const linkElement = document.getElementById("themeDefaultStyle");
                    ...
                    bazaar._renderReadme(target, (JSON.parse(target.getAttribute("data-obj")).bazaarType));
                    event.preventDefault();
                    event.stopPropagation();
                    break;
                }
        ...
        bazaar.element.querySelectorAll(".b3-select").forEach((selectElement) => {
    ...
    _onBazaar(response, bazaarType, reload) {
