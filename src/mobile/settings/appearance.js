...
export const initAppearance = (modelElement, modelMainElement) => {
    closePanel();
...
<div class="b3-label">
    ...
    modelMainElement.querySelector(".b3-slider").addEventListener("input", (event) => {
    ...
    modelMainElement.querySelectorAll("select").forEach(item => {
        item.addEventListener("change", () => {
            const modeElementValue = parseInt(modelMainElement.querySelector("#mode").value);
            fetchPost("/api/setting/setAppearance", Object.assign({}, window.siyuan.config.appearance, {
                icon: modelMainElement.querySelector("#icon").value,
                mode: modeElementValue === 2 ? window.siyuan.config.appearance.mode : modeElementValue,
                modeOS: modeElementValue === 2,
                themeDark: modelMainElement.querySelector("#themeDark").value,
                themeLight: modelMainElement.querySelector("#themeLight").value,
                lang: modelMainElement.querySelector("#lang").value,
            }), () => {
