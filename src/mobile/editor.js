...
export const openMobileFileById = (id, action = [Constants.CB_GET_HL]) => {
    ...
    if (window.siyuan.mobile.editor) {
        ...
        let blockElement;
        Array.from(window.siyuan.mobile.editor.protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${id}"]`)).find((item) => {
    ...
    fetchPost("/api/block/getBlockInfo", { id }, (data) => {
        ...
        document.getElementById("toolbarName").value = data.data.rootTitle === "Untitled" ? "" : data.data.rootTitle;
        setEditor();
        closePanel();
    });
};
