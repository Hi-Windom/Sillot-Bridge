...
const showAccountInfo = (modelElement, modelMainElement) => {
    closePanel();
    let userTitlesHTML = "";
...
const genWorkspace = (workspaceDirElement) => {
...
<div class="b3-form__space fn__none" id="form2">
    ...
    <button id="login2" class="b3-button fn__block">${window.siyuan.languages.login}</button>
                ...
                const agreeLoginElement = modelMainElement.querySelector("#agreeLogin");
                const userNameElement = modelMainElement.querySelector("#userName");
                const userPasswordElement = modelMainElement.querySelector("#userPassword");
                const captchaImgElement = modelMainElement.querySelector("#captchaImg");
                const captchaElement = modelMainElement.querySelector("#captcha");
                const twofactorAuthCodeElement = modelMainElement.querySelector("#twofactorAuthCode");
                const loginBtnElement = modelMainElement.querySelector("#login");
                const login2BtnElement = modelMainElement.querySelector("#login2");
                    ...
                    fetchPost("/api/account/login", {
                        ...
                        if (data.code === 1) {
                            ...
                            if (data.data.needCaptcha) {
                                ...
                                captchaElement.previousElementSibling.setAttribute("src", `https://ld246.com/captcha/login?needCaptcha=${data.data.needCaptcha}`);
...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.siyuan.languages.confirm}</button>
                ...
                const inputElement = initDialog.element.querySelector(".b3-text-field");
                inputElement.focus();
...
<div class="b3-dialog__action">
    ...
    modelMainElement.querySelector("#importData").addEventListener("change", (event) => {
        const formData = new FormData();
        formData.append("file", event.target.files[0]);
        fetchPost("/api/import/importData", formData);
    });
    const networkServeElement = modelMainElement.querySelector("#networkServe");
