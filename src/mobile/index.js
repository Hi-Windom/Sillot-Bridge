                ...
                fetchGet(`/appearance/langs/${window.siyuan.config.appearance.lang}.json?v=${Constants.SIYUAN_VERSION}`, (lauguages) => {
                    ...
                    fetchPost("/api/setting/getCloudUser", {}, userResponse => {
                        ...
                        fetchPost("/api/system/getEmojiConf", {}, emojiResponse => {
                            window.siyuan.emojis = emojiResponse.data;
                            initFramework();
                        });
                    });
                    addGA();
...
window.openFileByURL = (openURL) => {
    if (openURL && /^siyuan:\/\/blocks\/\d{14}-\w{7}/.test(openURL)) {
        openMobileFileById(openURL.substr(16, 22), getSearch("focus", openURL) === "1" ? [Constants.CB_GET_ALL, Constants.CB_GET_FOCUS] : [Constants.CB_GET_FOCUS, Constants.CB_GET_CONTEXT]);
        return true;
    }
    return false;
};
