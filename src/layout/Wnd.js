...
import { getInstanceById, getWndByLayout, JSONToCenter, newCenterEmptyTab, pdfIsLoading, resizeTabs, setPanelFocus, switchWnd } from "./util";
...
export class Wnd {
    ...
    <div class="layout-tab-container fn__flex-1">${dragHTML}</div>
        ...
        const dragElement = this.element.querySelector(".layout-tab-container__drag");
        if (!dragElement) {
            return;
        }
        ...
        this.headersElement.addEventListener("mousewheel", (event) => {
        ...
        this.headersElement.parentElement.addEventListener("dragover", function (event) {
            const it = this;
            ...
            let exitDrag = false;
            Array.from(it.firstElementChild.childNodes).find((item) => {
            ...
            if (!exitDrag && oldTabHeaderElement) {
                ...
                oldTabHeaderElement = oldTabHeaderElement.cloneNode(true);
        ...
        this.headersElement.parentElement.addEventListener("drop", function (event) {
            const it = this;
            if (event.dataTransfer.types.includes(Constants.SIYUAN_DROP_FILE)) {
                // 文档树拖拽
            ...
            let oldTab = getInstanceById(tabData.id);
            const wnd = getInstanceById(it.parentElement.getAttribute("data-id"));
            /// #if !BROWSER
            ...
            const nextTabHeaderElement = Array.from(it.firstElementChild.childNodes).find((item) => {
                if (item.style.opacity === "0.1") {
                    return true;
                }
            })?.nextElementSibling;
        ...
        dragElement.addEventListener("drop", (event) => {
            ...
            const targetWnd = getInstanceById(targetWndElement.getAttribute("data-id"));
            const tabData = JSON.parse(event.dataTransfer.getData(Constants.SIYUAN_DROP_TAB));
            let oldTab = getInstanceById(tabData.id);
            /// #if !BROWSER
            if (!oldTab) { // 从主窗口拖拽到页签新窗口
    ...
    showHeading() {
        const currentElement = this.headersElement.querySelector(".item--focus");
        if (!currentElement) {
            return;
        }
    ...
    switchTab(target, pushBack = false, update = true) {
        setPanelFocus(this.headersElement.parentElement.parentElement);
        let currentTab;
        ...
        if (currentTab && currentTab.model instanceof Editor) {
            ...
            if (keepCursorId) {
                ...
                let nodeElement;
                Array.from(currentTab.model.editor.protyle.wysiwyg.element.querySelectorAll(`[data-node-id="${keepCursorId}"]`)).find((item) => {
    ...
    renderTabList(event) {
        ...
        Array.from(this.headersElement.children).forEach((item) => {
            ...
            window.siyuan.menus.menu.append(new MenuItem({
                ...
                bind: (element) => {
                    element.addEventListener("click", (event) => {
                        if (hasClosestByTag(event.target, "svg")) {
    ...
    removeOverCounter(oldFocusIndex) {
    ...
    removeTabAction = (id, closeAll = false, hasSaveScroll = true, animate = true) => {
        clearCounter();
        this.children.find((item, index) => {
            if (item.id === id) {
                ...
                if (item.headElement) {
                    if (item.headElement.classList.contains("item--focus")) {
                        let latestHeadElement;
                        Array.from(item.headElement.parentElement.children).forEach((headItem) => {
    ...
    removeTab(id, closeAll = false, needSaveScroll = true, animate = true) {
    ...
    moveTab(tab, nextId) {
        ...
        if (oldWnd.children.length === 1) {
            ...
            if (!oldWnd.headersElement.querySelector(".item--focus")) {
                let latestHeadElement;
                Array.from(oldWnd.headersElement.children).forEach((headItem) => {
                    if (!latestHeadElement) {
                        latestHeadElement = headItem;
                    }
                    else if (headItem.getAttribute("data-activetime") > latestHeadElement.getAttribute("data-activetime")) {
