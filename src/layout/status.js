    ...
    document.querySelector("#status").addEventListener("click", (event) => {
        ...
        while (target.id !== "status") {
            if (target.id === "barDock") {
                ...
                JSON.parse(target.getAttribute("data-tasks")).forEach((item) => {
...
let countRootId;
export const countSelectWord = (range, rootID) => {
    /// #if !MOBILE
...
export const countBlockWord = (ids, rootID, clearCache = false) => {
    /// #if !MOBILE
    if (document.getElementById("status").classList.contains("fn__none")) {
        return;
    }
