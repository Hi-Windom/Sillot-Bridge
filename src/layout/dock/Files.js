...
<ul class="b3-list fn__flex-column" style="min-height: auto;transition: var(--b3-transition)">
    ...
    <ul class="fn__none fn__flex-1"></ul>
        ...
        this.actionsElement = options.tab.panelElement.firstElementChild;
        this.element = this.actionsElement.nextElementSibling;
        this.closeElement = options.tab.panelElement.lastElementChild;
        this.closeElement.addEventListener("click", (event) => {
            ...
            while (target && !target.isEqualNode(this.closeElement)) {
                ...
                if (target.classList.contains("b3-list-item__icon")) {
                    ...
                    confirmDialog(window.siyuan.languages.deleteOpConfirm, `${window.siyuan.languages.confirmDelete} <b>${escapeHtml(target.parentElement.querySelector(".b3-list-item__text").textContent)}</b>?`, () => {
        ...
        this.actionsElement.addEventListener("click", (event) => {
            let target = event.target;
        ...
        this.element.addEventListener("dragstart", (event) => {
            ...
            if (liElement) {
                let selectElements = Array.from(this.element.querySelectorAll(".b3-list-item--focus"));
        ...
        this.element.addEventListener("dragover", (event) => {
            ...
            Array.from(this.element.querySelectorAll(".b3-list-item--focus")).find((item) => {
        ...
        this.element.addEventListener("drop", async (event) => {
            ...
            const selectRootElements = [];
            const selectFileElements = [];
            const fromPaths = [];
            this.element.querySelectorAll(".b3-list-item--focus").forEach((item) => {
...
<li class="b3-list-item b3-list-item--hide-action" draggable="true" data-type="navigation-root" data-path="/">
    ...
    onRemove(data) {
        ...
        data.data.ids.forEach((item) => {
            ...
            if (targetElement) {
                ...
                // 移除当前节点
                const parentElement = targetElement.parentElement.previousElementSibling;
    ...
    onMount(data) {
        ...
        const liElement = this.closeElement.querySelector(`li[data-url="${data.data.box.id}"]`);
        if (liElement) {
            liElement.remove();
        }
        setNoteBook((notebooks) => {
    ...
    onMove(data) {
        const sourceElement = this.element.querySelector(`ul[data-url="${data.fromNotebook}"] li[data-path="${data.fromPath}"]`);
        if (sourceElement) {
        ...
        const newElement = this.element.querySelector(`[data-url="${data.toNotebook}"] li[data-path="${data.toPath}"]`);
        // 更新移动到的新文件夹
        if (newElement) {
    ...
    setCurrent(target, isScroll = true) {
        if (!target) {
            return;
        }
    ...
    selectItem(notebookId, filePath, data) {
...
class="b3-list-item b3-list-item--hide-action" data-path="${item.path}">
    ...
    initMoreMenu() {
        ...
        if (!window.siyuan.config.readonly) {
            sortMenu("notebooks", window.siyuan.config.fileTree.sort, (sort) => {
