...
<div class="backlinkMList fn__flex-1"></div>`;
        ...
        this.tree = new Tree({
            element: this.element.querySelector(".backlinkList"),
            data: null,
            click: (element) => {
        ...
        this.mTree = new Tree({
            element: this.element.querySelector(".backlinkMList"),
            data: null,
            click: (element) => {
        ...
        this.element.querySelector('[data-type="expand"]').addEventListener("click", () => {
            Array.from(this.tree.element.firstElementChild.children).forEach((item) => {
                ...
                if (target.classList.contains("block__icon") && target.parentElement.parentElement.isSameNode(this.element)) {
                    ...
                    switch (type) {
                        case "refresh":
                            ...
                            break;
                        case "mExpand":
                            Array.from(this.mTree.element.firstElementChild.children).forEach((item) => {
                                if (item.tagName === "LI" && !item.querySelector(".b3-list-item__arrow--open")) {
    ...
    showSortMenu(type, sort) {
        const clickEvent = (currentSort) => {
    ...
    toggleItem(liElement, isMention) {
    ...
    render(data) {
        ...
        this.status[this.blockId].backlinkOpenIds.forEach(item => {
            const liElement = this.tree.element.querySelector(`.b3-list-item[data-node-id="${item}"]`);
            if (liElement) {
                this.toggleItem(liElement, false);
            }
        });
        this.status[this.blockId].backlinkMOpenIds.forEach(item => {
            const liElement = this.mTree.element.querySelector(`.b3-list-item[data-node-id="${item}"]`);
            if (liElement) {
                this.toggleItem(liElement, true);
            }
        });
