...
<div class="fn__flex-1 inbox__list"></div>`;
        ...
        const selectAllElement = this.element.querySelector(".block__icons input");
                ...
                if (typeElement && this.element.contains(typeElement)) {
                    ...
                    switch (type) {
                        ...
                        case "selectall":
                            ...
                            break;
                        case "select":
                            if (typeElement.firstElementChild.nextElementSibling.checked) {
...
${(Lute.New()).MarkdownStr("", data.shorthandContent)}
    ...
    <li class="b3-list--empty">
        ...
        fetchPost("/api/inbox/getShorthands", { page: this.currentPage }, (response) => {
            ...
            if (response.data.data.shorthands.length === 0) {
                ...
                response.data.data.shorthands.forEach((item) => {
    ...
    <span class="b3-list-item__meta">${item.hCreated}</span>
            ...
            this.element.querySelector(".block__icons input").checked = this.element.lastElementChild.querySelectorAll("input:checked").length === selectCount && selectCount !== 0;
        });
    }
}
