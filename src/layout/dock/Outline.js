...
<div class="fn__flex-1" style="margin-bottom: 8px"></div>`;
        this.element = options.tab.panelElement.lastElementChild;
        this.headerElement = options.tab.panelElement.firstElementChild;
        this.tree = new Tree({
            element: options.tab.panelElement.lastElementChild,
            data: null,
            click: (element) => {
        ...
        options.tab.panelElement.querySelector('[data-type="expand"]').addEventListener("click", (event) => {
        ...
        options.tab.panelElement.addEventListener("click", (event) => {
            if (this.type === "local") {
...
<span class="b3-list-item__text">${escapeHtml(ial.title)}</span>`;
    ...
    onTransaction(data) {
        ...
        data.data[0].doOperations.forEach((item) => {
        ...
        if (data.data[0].undoOperations) {
            data.data[0].undoOperations.forEach((item) => {
        ...
        if (nodeElement.getAttribute("data-type") === "NodeHeading") {
            ...
            if (previousElement) {
                ...
                fetchPost("/api/block/getBlockBreadcrumb", {
                    ...
                    response.data.reverse().find((item) => {
